...
func (r router) handleRegisterExchange(w http.ResponseWriter, req *http.Request) {
	dto := &dtos.Exchange{}
	...
	_, code, err := r.app.Auth.Sessions["register"].Get(u.Uuid)
	if err != nil {
		logger.Error(err)
	...
	if code != dto.Code {
		logger.WarnC(fmt.Sprintf("User with Email: %s provided wrong token during register", dto.Email))
		http.Error(w, "Invalid Code", http.StatusBadRequest)
		return
	}
	_, err = r.app.Data.UpdateUserIsDraft(req.Context(), data.UpdateUserIsDraftParams{IsDraft: false, ID: u.ID})
	if err != nil {
		logger.Error(err)
		http.Error(w, "", http.StatusBadRequest)
		return
	...
	r.app.Auth.Sessions["register"].Remove(u.Email.String)
}
